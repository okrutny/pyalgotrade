from kraken import common
from pyalgotrade.kraken import barfeed
...
class Strategy(strategy.BaseStrategy):
    ...
    def onBars(self, bars):
        ...
        self.info("onBars: Price: %s. Volume: %s." % (bar.getClose(), bar.getVolume()))

        # Wait until we get the current bid/ask prices.
        if self.__ask is None or self.__bid is None:
            return
        ...
        elif self.temp_counter == 10:
            self.info("Entry signal. Sell at %s" % (self.__bid))
            self.__position = self.enterShortLimit(self.__instrument, self.__bid, self.__posSize, True)
